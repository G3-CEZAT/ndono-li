...
export const TopHeader: React.FC<TopHeaderProps> = ({
  ...
  const getTabDetails = () => {
    switch (activeTab) {
      case 'pilgrim-home':
        return {
          title: "Page d'Accueil Pèlerin — Hadara Malikiyya de Tivaouane",
          description: "Médiation numérique, biographie de Maodo, projet des 4 piliers, Gamou 2026 et assistant doctrinal RAG.",
          badge: "Portail Mobile & Web",
        };
      case 'dashboard':
        return {
          title: "Hadara Tidiane — Station d'Arbitrage & Administration",
          description: "Vue d'ensemble de la médiation RAG, héritage de Maodo, flux des requêtes et arbitrages des érudits.",
          badge: "Espace Érudits Actif",
        };
      case 'queue':
        return {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 self-end md:self-auto">
        ...
        {/* Return to Pilgrim Portal Button */}
        {onReturnToPilgrimView && (
          <button
            onClick={onReturnToPilgrimView}
            className="px-3 py-2 rounded-lg bg-[#D4A72C] hover:bg-[#E8C158] text-[#0E4D3C] font-bold text-xs shadow-xs transition-colors flex items-center gap-1.5 whitespace-nowrap"
          >
            <span>← Espace Pèlerins</span>
          </button>
        )}
